...
public class CollisionSystem extends GameSystem{
	...
	private void collidePair(GameObject go1, GameObject go2, Vec2d mtv) {
		if(go1.getName().contains("ENEMY") && go2.getName().equals("PLAYER")) {
			...
			AbilityCollisionComponent curr = (AbilityCollisionComponent) go1.getComponent("COLLISION");
			curr.hit(go2);
		} else if (go1.getName().equals("WALL")) {
			if (!go2.getName().equals("WALL") && !go2.getName().contains("ABILITY")) {
				TransformComponent m2 = (TransformComponent) go2.getComponent("TRANSFORM");
				m2.move(mtv.sdiv(-1));
			}
		} else if (go1.getName().equals("ABILITY") && go2.getName().equals("ENABILITY")) {
			//AbilityCollisionComponent curr = (AbilityCollisionComponent) go2.getComponent("COLLISION");
			//curr.removeHitBox();
		}
	}
}
